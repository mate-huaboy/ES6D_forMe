...
# bind loss (R^T@t)
def loss_by_bind(out_rot,gt_rot,out_trans,gt_trans,BIND_LOSS_TYPE="L1"):
    _,c,_,_=out_rot.shape
    pred_bind = torch.matmul(out_rot.permute(0,1, 3, 2), out_trans.view(-1,1, 3, 1)).view(-1,c, 3)
    gt_bind = torch.matmul(gt_rot.permute(0,1, 3, 2), gt_trans.view(-1, 1,3, 1)).view(-1,c, 3)
    if BIND_LOSS_TYPE == "L1":
        loss_dict = nn.L1Loss(reduction="none")(pred_bind, gt_bind)
    elif BIND_LOSS_TYPE == "L2":
        loss_dict = L2Loss(reduction="mean")(pred_bind, gt_bind)
    elif BIND_LOSS_TYPE == "MSE":
        loss_dict = nn.MSELoss(reduction="mean")(pred_bind, gt_bind)
    else:
